...
const COLORS={'Perumahan Bukan Strata':'#f59e0b','Perumahan Strata':'#244a7c','Kampung':'#ef5b23','Setinggan':'#dc2626','Tidak Dinyatakan':'#9ca3af'};
const PALETTE=['#f59e0b','#244a7c','#ef5b23','#dc2626','#8b3fa9','#fbbf24','#9ca3af'];
...
function update(){const rows=filteredRows();const total=sum(rows,'count');$('kpiRecords').textContent=fmt(total);if($('donutTotal'))$('donutTotal').textContent=fmt(total);$('kpiArea').textContent=fmtArea(sum(rows,'area'));$('kpiDistricts').textContent=new Set(rows.map(r=>r.district)).size;$('kpiPbts').textContent=new Set(rows.map(r=>r.pbt)).size;updateTypeChart(rows);updateDistrictChart(rows);updateSubtypeChart(rows);renderTable()}
